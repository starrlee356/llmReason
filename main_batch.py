...
from Infer_Answer_batch import infer_and_answer
...
def main():    
    ...
    def answer(qtype, qpattern, batch_size):
        ...
        iters = list(chunked(idx_list, batch_size))
        for batch_idx in tqdm(iters, desc=f"{qtype} {qpattern} {des}", total=len(iters)):
            batch_query = [idx_to_queries[qtype][idx] for idx in batch_idx]
            model.answer_query(batch_query, qtype, batch_idx, output_path=pred_path)
    ...
    if args.qtype == "all": 
        for qtype, qpattern in model.q_structs.items():
            answer(qtype=qtype, qpattern=qpattern, batch_size=args.batch_size)
    else: 
        for qtype, qpattern in model.q_structs.items():
            if qtype == args.qtype:
                answer(qtype=qtype, qpattern=qpattern, batch_size=args.batch_size)
...
if __name__=="__main__":
    ...
    parser.add_argument('--batch_size', type=int, default=1, help="batch size for LLM infer")
