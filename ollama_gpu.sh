...
validate_input() {
    if [[ ! $1 =~ ^[0-7](,[0-7])*$ ]]; then
        echo "Error: Invalid input. Please enter numbers between 0 and 7, separated by commas."
        exit 1
    fi
}
...
if [ "$#" -eq 0 ]; then
    ...
    read -p "Enter CUDA_VISIBLE_DEVICES values (0-7, comma-separated): " cuda_values
